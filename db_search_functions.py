...
class DBsearch:
    def __init__(self):
        ...
        self.conn.begin()
        config.MENU_LOGGER.debug(f"DBsearch class instantiated. Connection to the database successfully opened. ")
    ...
    def get_company_uid_from_name(self, name):
        ...
        try:
            ...
            print(e)
            config.MENU_LOGGER.critical(f"{e}")
            raise SQLAlchemyError(f"{e._message()}")
        except DBAPIError as e2:
            print(e2)
            config.MENU_LOGGER.critical(f"{e2}")
            raise DBAPIError(f"{e2.message()}")


        config.MENU_LOGGER.debug(f"Successful execution of an SQL query to the database")
        for result in results:
            uid = result[0]

        config.MENU_LOGGER.debug(f"Returning (uid=): {uid}")
        return uid
    ...
    def get_company_name_from_uid(self, uid):
        ...
        config.MENU_LOGGER.debug(f"Successful execution of an SQL query to the database")
        for result in results:
            config.MENU_LOGGER.debug(f"Returning (name=): {result[0]}")
            return result[0]
    ...
    def get_company_info(self, uid):
        ...
        for result in results:
            ...
            config.MENU_LOGGER.debug(f"Returning (info=): {info} ")
            return info
    ...
    def get_company_description(self, uid):
        ...
        for result in results:
            config.MENU_LOGGER.debug(f"Returning (description)")
            if result[0] is None:
                return ''
    ...
    def get_companies_by_loc(self, locations):
        ...
        for location in locations:
            config.MENU_LOGGER.debug(f"Searching for companies located in {location}")
        ...
        config.MENU_LOGGER.debug(f"Returning (company names=): {company_names}")
    ...
    def get_dpl_profile(self, uid):
        ...
        config.MENU_LOGGER.debug(f"Returning (companies dpl profiles=): {result_dic}")
        return result_dic
    ...
    def comp_search_db(self, search_param, display_params, posit_disp_params):
        ...
        """""
        config.MENU_LOGGER.info(f"Entering company search function comp_search_db.")
        ...
        for company_name in company_names:
            config.MENU_LOGGER.debug(f"Beginning to collect info from the database for company: {company_name}")
            ...
            config.MENU_LOGGER.debug(f"Calling  {self.get_company_info} with argument {uid}")
            info = self.get_company_info(uid)

            if display_params['pdl'] or display_params['all']:
                config.MENU_LOGGER.debug(f"Calling  {self.get_dpl_profile} with argument {uid}")
            ...
            if display_params['description'] or display_params['all']:
                config.MENU_LOGGER.debug(f"Calling  {self.get_company_description} with argument {uid}")
            ...
            if display_params['positions'] or display_params['all']:
                ...
                config.MENU_LOGGER.debug(f"Calling  {self.posit_search_db} with arguments name_dic= {name_dic} and posit_disp_params={posit_disp_params}")
    ...
    def create_posit_where_clause(self, posit_params):
        ...
        if posit_params['company'] is not None:
            for comp_name in posit_params['company']:
                config.MENU_LOGGER.debug(f"Calling {self.get_company_uid_from_name} for company: {comp_name}")
        ...
        config.MENU_LOGGER.debug(f"Returning {clause}")
        return clause
    ...
    def get_position_description(self, uid):
        ...
        config.MENU_LOGGER.debug(f"Successful execution of an SQL query to the database")
        for result in results:
            config.MENU_LOGGER.debug(f"Returning")
            if result[0] is None:
                return ''
            return html2text.html2text(result[0])
    ...
    def posit_search_db(self, posit_params, posit_display_params):
        """
           ...
           :return:
        """
        config.MENU_LOGGER.info(f"Entering positions search function comp_search_db.")
        config.MENU_LOGGER.debug(f"Calling {self.create_posit_where_clause} with argument {posit_params}")
        ...
        for name, positions in companies:
            config.MENU_LOGGER.debug(f"Calling {self.get_company_name_from_uid} with argument {name}")
            ...
            for i, r in positions.iterrows():
                ...
                if posit_display_params['description'] or posit_display_params['all']:
                    print('\tDescription:')
                    config.MENU_LOGGER.debug(f"Calling {self.get_position_description} with argument {r['position_uid']}")
                    print(self.get_position_description(r['position_uid']))
                if posit_display_params['updated'] or posit_display_params['all']:
                    print('\tTime updated:', r['time_updated'])
